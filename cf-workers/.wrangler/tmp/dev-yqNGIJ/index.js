...
// .wrangler/tmp/bundle-s1BT3Y/strip-cf-connecting-ip-header.js
...
async function handleLive(channelId, sessionData, config, baseUrl, env, request) {
    ...
    for (const raw of lines) {
      ...
      if (line.includes('URI="')) {
        ...
        if (uriMatch) {
          ...
          out.push(line.replace(`URI="${keyUrl}"`, `URI="${baseUrl}/wanda.php?token=${JIOTV_TOKEN2}&id=${channelId}&thor=${thorB64}&pkey=${encKeyUrl}"`));
        } else {
          out.push(line);
        }
      } else if (line.includes(".ts") && !line.startsWith("#")) {
...
// .wrangler/tmp/bundle-s1BT3Y/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-s1BT3Y/middleware-loader.entry.ts
